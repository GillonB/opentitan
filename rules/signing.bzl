...
load("//rules:manifest.bzl", "ManifestInfo")
...
def _presigning_artifacts(ctx, opentitantool, src, manifest, ecdsa_key, rsa_key, spx_key, owner_ecdsa_key = None, owner_spx_key = None, basename = None, keyname_in_filenames = False):
    ...
    if hasattr(ctx.attr, "manifest") and ctx.attr.manifest:
        for f in ctx.attr.manifest[DefaultInfo].files.to_list():
            if f not in inputs:
                inputs.append(f)
        if ManifestInfo in ctx.attr.manifest:
            for f in ctx.attr.manifest[ManifestInfo].keys.to_list():
                if f not in inputs:
                    inputs.append(f)
...
def _offline_presigning_artifacts(ctx):
    ...
    if not owner_ecdsa_key:
        owner_ecdsa_key = ecdsa_key
    owner_spx_key = key_from_dict(ctx.attr.owner_spx_key, "owner_spx_key")
    if not owner_spx_key:
        owner_spx_key = spx_key
    digests = []
    bins = []
    script = []
...
def sign_binary(ctx, opentitantool, **kwargs):
    ...
    owner_ecdsa_key_attr = None
    if "owner_ecdsa_key" in kwargs:
        owner_ecdsa_key_attr = kwargs["owner_ecdsa_key"]
    elif hasattr(ctx.attr, "owner_ecdsa_key"):
        owner_ecdsa_key_attr = ctx.attr.owner_ecdsa_key
    owner_ecdsa_key = key_from_dict(owner_ecdsa_key_attr, "owner_ecdsa_key")
    if not owner_ecdsa_key:
        owner_ecdsa_key = ecdsa_key

    owner_spx_key_attr = None
    if "owner_spx_key" in kwargs:
        owner_spx_key_attr = kwargs["owner_spx_key"]
    elif hasattr(ctx.attr, "owner_spx_key"):
        owner_spx_key_attr = ctx.attr.owner_spx_key
    owner_spx_key = key_from_dict(owner_spx_key_attr, "owner_spx_key")
    if not owner_spx_key:
        owner_spx_key = spx_key
