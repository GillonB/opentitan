...
static void hmac_configure_digest(bool big_endian_digest, bool hmac_mode,
  ...
  abs_mmio_write32(hmac_base() + HMAC_INTR_ENABLE_REG_OFFSET, 0u);
  abs_mmio_write32(hmac_base() + HMAC_INTR_STATE_REG_OFFSET, UINT32_MAX);
...
static void hmac_configure(bool big_endian_digest, bool hmac_mode) {
  hmac_configure_digest(big_endian_digest, hmac_mode,
...
void hmac_sha384_configure(bool big_endian_digest) {
  hmac_configure_digest(big_endian_digest, /*hmac_mode=*/false,
...
void hmac_sha256_update(const void *data, size_t len) {
  ...
  for (; len != 0 && (uintptr_t)data_sent & 3; --len) {
    abs_mmio_write8(hmac_base() + HMAC_MSG_FIFO_REG_OFFSET, *data_sent++);
  }

  for (; len >= sizeof(uint32_t); len -= sizeof(uint32_t)) {
    uint32_t data_aligned = read_32(data_sent);
    abs_mmio_write32(hmac_base() + HMAC_MSG_FIFO_REG_OFFSET, data_aligned);
...
void hmac_sha256_update_words(const uint32_t *data, size_t len) {
  for (size_t i = 0; i < len; i++) {
    abs_mmio_write32(hmac_base() + HMAC_MSG_FIFO_REG_OFFSET, data[i]);
  }
}
...
void hmac_sha256_final_truncated(uint32_t *digest, size_t len) {
  ...
  uint32_t reg = abs_mmio_read32(hmac_base() + HMAC_CFG_REG_OFFSET);
...
void hmac_sha384_final(hmac_digest_sha384_t *digest) {
  ...
  if (bitfield_bit32_read(reg, HMAC_CFG_DIGEST_SWAP_BIT)) {
    ...
    // Little-endian output.
    result = HMAC_DIGEST_0_REG_OFFSET +
             (kHmacDigestSha384NumWords - 1) * sizeof(uint32_t);
    incr = (uint32_t)-sizeof(uint32_t);
  }
...
void hmac_sha256_save(hmac_context_t *ctx) {
  ...
  for (uint32_t i = 0; i < kHmacDigestNumWords; i++) {
    ctx->digest[i] = abs_mmio_read32(hmac_base() + HMAC_DIGEST_0_REG_OFFSET +
                                     i * sizeof(uint32_t));
  }

  // Read the message length registers.
  ctx->msg_len_lower =
      abs_mmio_read32(hmac_base() + HMAC_MSG_LENGTH_LOWER_REG_OFFSET);
  ctx->msg_len_upper =
      abs_mmio_read32(hmac_base() + HMAC_MSG_LENGTH_UPPER_REG_OFFSET);

  // Momentarily clear the `sha_en` bit, which clears the digest.
  uint32_t cfg = abs_mmio_read32(hmac_base() + HMAC_CFG_REG_OFFSET);
  abs_mmio_write32(hmac_base() + HMAC_CFG_REG_OFFSET,
...
void hmac_sha256_restore(const hmac_context_t *ctx) {
  ...
  for (uint32_t i = 0; i < kHmacDigestNumWords; i++) {
    abs_mmio_write32(
        hmac_base() + HMAC_DIGEST_0_REG_OFFSET + i * sizeof(uint32_t),
        ctx->digest[i]);
  }

  // Write the message length registers.
  abs_mmio_write32(hmac_base() + HMAC_MSG_LENGTH_LOWER_REG_OFFSET,
                   ctx->msg_len_lower);
  abs_mmio_write32(hmac_base() + HMAC_MSG_LENGTH_UPPER_REG_OFFSET,
                   ctx->msg_len_upper);
