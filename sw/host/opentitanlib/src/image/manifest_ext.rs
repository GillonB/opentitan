...
fn default_cert_type_leaf() -> u32 {
    0x4c454146 // ASCII 'LEAF'
}
...
pub enum ManifestExtEntrySpec {
    ...
    DelegationCert {
        #[serde(default = "default_cert_type_leaf")]
        cert_type: u32,
        version: u32,
        owner_key_id: u32,
...
";
    ...
    fn test_delegation_cert_spec_parse() {
        ...
        if let ManifestExtEntrySpec::DelegationCert {
            ...
            assert_eq!(*cert_type, 0x4c454146);
            assert_eq!(*version, 1);
            assert_eq!(*owner_key_id, 2);
            assert_eq!(
                delegate_public_key,
                &PathOrRaw::Path(PathBuf::from("test_delegate.pub.der"))
            );
            assert_eq!(device_id, &[1, 2, 3, 4, 5, 6, 7, 8]);
            assert_eq!(
                owner_signature,
                &Some(PathOrRaw::Path(PathBuf::from("test_owner.sig")))
            );
        } else {
        ...
        if let ManifestExtEntrySpec::DelegationCertSpx {
            ...
            assert_eq!(
                delegate_spx_key,
                &PathOrRaw::Path(PathBuf::from("test_spx.pem"))
            );
            assert_eq!(
                signature,
                &Some(PathOrRaw::Path(PathBuf::from("test_spx.sig")))
            );
        } else {
    ...
    fn test_delegation_cert_binary_generation() -> Result<()> {
        ...
        assert_eq!(u32::from_le_bytes(bytes[8..12].try_into().unwrap()), 0x4c454146); // cert_type
        assert_eq!(u32::from_le_bytes(bytes[12..16].try_into().unwrap()), 1); // version
        assert_eq!(u32::from_le_bytes(bytes[16..20].try_into().unwrap()), 2); // owner_key_id
